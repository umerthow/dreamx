           ...
           <?php if($npage==26){ ?>
            $('.table-subscribe').DataTable();
             ...
             $(document).on("click",".export-xls",function(){
                   ...
                   $.ajax({
                  type:"POST",
                  url:"<?php echo base_url('AN_ajax_admin/export_subscribe'); ?>",
                  cache: false,
                  dataType:"json",
                  success: function(a){
                     alert('success');
            ...
            $(document).on("click",".hapus-pesan,.tombol-hapus-pesan",function(){
                    ...
                    konfirmasi("Hapus Pesan","Yakin akan menghapus data ini?",hapus_pesan,id);
            });
              ...
              function hapus_pesan(id){  
                ...
                $.ajax({
                  type:"POST",
                  data:{id:id},
                  url:"<?php echo base_url('AN_ajax_admin/hapus_subscribe'); ?>",
                  cache: false,
                  dataType:"json",
                  success: function(a){

                    $(".table-subscribe").find("tr[data-id='"+id+"']").hide("slow",function(){
                        $(this).remove();
                    });                 
                  },
                  error: function(a,b,c){
        ...
        foto_artikel.on("success",function(a,b){
           ...
           $(".save-artikel").click(function(){
            ...
            var ada_foto=((<?php echo $artikel_id ?>==0 && j_foto_yaang_diupload>0) || (<?php echo $artikel_id ?>>0 && $(".aPhotoThumb").length >0 ));
              ...
              var img_courtesy_a = $("#img_courtesy").val();
          ...
          <?php
           ...
           ?>           
            ...
            $.ajax({
              ...
              data:{"id":id_page,"judul":judul_page,"isi_page":isi_page,"url":url_seo,"image":featured_image,"status":status_page,"js":js_page,"keywords":keywords,"description":description,"highlight":highlight},
              cache:false,
              success:function(a){
                if(a=="ok"){
                  show_proses("Berhasil! Mereload halaman...");
