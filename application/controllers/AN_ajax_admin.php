...
class AN_ajax_admin extends CI_Controller
{
	...
	function submit_artikel($i=0){
		...
		$courtesy = changequote($this->input->post('img_courtesy'));
	...
	function page(){
		if($this->session->userdata("level_user")==1){
			...
			$highlight=changequote($this->input->post("highlight"));
			$time=date("Y:m:d H:i:s",now());


			if($id==0){
				// "baru";
				$query=$this->db->query("INSERT INTO pages (page_judul,page_url,page_isi,page_foto,page_javascript,page_status,page_id_user,page_created,page_meta_keywords,page_meta_description,highlight) VALUES ('$judul','$url','$isi','$image','$js','$status','$user','$time','$keywords','$description','$highlight')");
	...
	function hapus_subscribe(){
		$id=$this->input->post("id");
		$this->db->delete("tsubscribe",array("noid"=>$id));
		echo "{}";
	}
